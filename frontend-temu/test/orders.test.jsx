...
import { TestComponentInitial, TestUserContext } from './TestComponent.jsx';
...
describe('Cancelar y modificar pedido', () => {
...
describe('Link y seguimiento de pedido', () => {
    test('Link de pedido', async () => {
        axios.get.mockResolvedValue({
            data: [order],
        });
        ...
        await waitFor(async () => {
            expect(
                screen.getByText(/Productos del pedido/i, { exact: false }),
            ).toBeInTheDocument();

            expect(
                screen.getByText(order.envio.departamento, { exact: false }),
            ).toBeInTheDocument();
        });
    });
});
